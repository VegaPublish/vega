...
class CorsCheck extends PureComponent {
  ...
  render() {
      ...
      if (is404 && errType === 'project') {
        return (
          <div>
            ...
            <p>
              Double-check that your <code>lyra.json</code> points to the right
              project ID?
            </p>
          </div>
        )
    ...
    if (result.isCorsError) {
      return this.renderWrapper(
        <p>
          ...
          <code>{origin}</code>) not being an allowed origin for this project.
          If you are an administrator or developer of the project, you can check
          the backend CORS configuration interface and add {origin} to the list
          of accepted <em>CORS Origins</em>.
        </p>
      )
    }
    ...
    if (result.pingResponded) {
      return this.renderWrapper(
        <p>
          The cause of this error is a little uncertain. It could be a temporary
          glitch, in which case you might want to try hitting the{' '}
          <strong>Retry</strong> button below. If you are the developer of this
          project, you could take a look at the browsers developer console and
          see if any issues are reported there.
        </p>
      )
    }
    ...
    return this.renderWrapper(
      <p>
        ...
        <em>might</em> be the Lyra API is having some issues, in which case it
        should hopefully be back up soon! You could also try hitting the{' '}
        <strong>Retry</strong> button and see if it was just a temporary glitch.
      </p>
    )
  }
}
