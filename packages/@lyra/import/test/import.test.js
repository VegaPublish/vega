...
const {getLyraClient} = require('./helpers')

const defaultClient = lyraClient({
  host: 'http://lyra.api',
  dataset: 'bar',
  useCdn: false,
  token: 'foo'
...
test('rejects on invalid JSON', async () => {
  expect.assertions(1)
  await expect(
    importer(getFixtureStream('invalid-json'), importOptions)
...
test('rejects on invalid `_id` property', async () => {
  expect.assertions(1)
  await expect(
    importer(getFixtureStream('invalid-id'), importOptions)
  ).rejects.toHaveProperty(
    'message',
...
test('rejects on missing `_type` property', async () => {
  expect.assertions(1)
  await expect(
    importer(getFixtureStream('missing-type'), importOptions)
  ).rejects.toHaveProperty(
    'message',
...
test('rejects on missing `_type` property (from array)', async () => {
  expect.assertions(1)
  await expect(
    importer(getFixtureArray('missing-type'), importOptions)
  ).rejects.toHaveProperty(
    'message',
...
test('accepts an array as source', async () => {
  ...
  const client = getLyraClient(getMockMutationHandler())
...
test('generates uuids for documents without id', async () => {
  ...
  const client = getLyraClient(getMockMutationHandler(match))
  const res = await importer(getFixtureStream('valid-but-missing-ids'), {
    client
  })
  expect(res).toBe(3)
})
