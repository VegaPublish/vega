...
const debug = require('debug')('lyra:import:folder')
...
module.exports = async function importFromFolder(fromDir, options, importers) {
  ...
  if (dataFiles.length > 1) {
    throw new Error(
      `More than one .ndjson file found in ${fromDir} - only one is supported`
    )
  }

  const assetMap = await fse
    .readJson(path.join(fromDir, 'assets.json'))
    .catch(() => ({}))
