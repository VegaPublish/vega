const debug = require('debug')('lyra:import:array')
...
async function importDocuments(documents, options, importers) {
  ...
  const absPathed = documents.map(doc =>
    absolutifyPaths(doc, options.assetsBase)
  )
  ...
  // Lyra prefers to have a `_type` on every object. Make sure references
