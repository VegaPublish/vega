...
function fetchAllDocumentPaths(selections: Selection[]) {
  ...
  return client.observable
    .fetch(toVegaQLQuery(combinedSelections))
    .pipe(map(result => reassemble(result, combinedSelections)))
}

const reloadFields = debounceCollect(fetchAllDocumentPaths, 0)

function listenFields(id: Id, fields: FieldName[]) {
  return listen(id).pipe(
    switchMap(event => {
      if (event.type === 'welcome') {
        return reloadFields(id, fields).pipe(
          mergeMap(result => {
            return concat(
              observableOf(result),
              result === undefined
                ? // hack: if we get undefined as result here it can be because the document has
                  ...
                  // and then re-fetch.
                  reloadFields(id, fields)
                : []
            )
          })
        )
      }
      return reloadFields(id, fields)
    })
  )
}
...
export default function cachedObserveFields(id: Id, fields: FieldName[]) {
  ...
  const missingFields = difference(
    fields,
    flatten(
      existingObservers.map(cachedFieldObserver => cachedFieldObserver.fields)
    )
  )
  ...
  const cachedFieldObservers = existingObservers
    .filter(observer =>
      observer.fields.some(fieldName => fields.includes(fieldName))
    )
  ...
  return combineLatest(cachedFieldObservers).pipe(
    ...
    map(snapshots =>
      snapshots.length === 0 ? null : pickFrom(snapshots, fields)
    ),
