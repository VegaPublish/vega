...
import {escapeField, fieldNeedsEscape} from 'part:@lyra/base/util/search-utils'
...
const maybeEscape = fieldName =>
  fieldNeedsEscape(fieldName)
    ? `"${fieldName}": @${escapeField(fieldName)}`
    : fieldName

function toSubQuery({ids, fields}) {
  const allFields = [...INCLUDE_FIELDS_QUERY, ...fields]
  return `*[_id in [${ids.map(stringifyId).join(',')}]][0...${
    ids.length
  }]{${allFields.map(maybeEscape).join(',')}}`
}

export function toVegaQLQuery(combinedSelections: CombinedSelection[]) {
  return `[${combinedSelections.map(toSubQuery).join(',')}][0...${
