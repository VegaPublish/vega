...
const reportErrors = debounce(() => {
  ...
  Object.keys(errorsByType).forEach(typeName => {
    ...
    console.group(
      `Check the preview config for schema type "${typeName}": %o`,
      first.type.preview
    )
...
function inspect(val, prefixType = true) {
  ...
  if (Array.isArray(val)) {
    ...
    return `${prefixType ? prefix : ''}[${val.map(v =>
      inspect(v, false)
    )}${ellipse}]`
  }
...
export const invokePrepare = __DEV__
  ? invokePrepareChecked
  : invokePrepareUnchecked
...
export default function prepareForPreview(
  ...
  const returnValueResult = validateReturnedPreview(
    invokePrepare(type, selectedValue, viewOptions)
  )
