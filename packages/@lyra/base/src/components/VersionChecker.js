...
import versions from 'lyra:versions'
...
if (fakeOutdatedModule) {
  versions['@lyra/base'] = '0.118.0'
  applySeverity = inp => {
    const mod =
      (inp && inp.outdated.find(item => item.name === '@lyra/base')) || {}
...
const buildQueryString = () => ({
  m: Object.keys(versions).map(pkg => `${pkg}@${versions[pkg]}`)
})
...
const onVersionCheckError = err => {
  ...
  console.warn(
    'Module versions check failed. Dependencies *might* be out of date.',
    err
  )
}
      ...
      >
        ...
        {result.helpUrl && (
          <p>
            ...
            <a href={result.helpUrl}>{result.helpUrl}</a>
          </p>
        )}
      </Dialog>
    )
