...
import projectFetcher from 'part:@lyra/base/project-fetcher'
...
function handleApiError(errObj, observer) {
  if (errObj.code === 'ACCESS_DENIED') {
    return observer.error(
      new AccessDeniedError('You are not allowed to perform this action.')
    )
  }
    ...
    return observer.error(
      new NotFoundError(
        `An project with label '${
          config.api.dataset
        }' not found. Check your project's lyra.json file.`
      )
    )
  }
