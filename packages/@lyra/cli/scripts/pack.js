...
const compiler = webpack({
  mode: 'production',
  entry: {
    lyra: path.join(basedir, 'bin/lyra.js')
  },
  output: {
    pathinfo: true,
    filename: 'lyra',
  ...
  plugins: [
    ...
    new webpack.DefinePlugin({
      __LYRA_IS_BUNDLED__: JSON.stringify(true),
...
compiler.run((err, stats) => {
  ...
  const filtered = stats.compilation.warnings.filter(warn => {
    return (
      !warn.origin || warn.origin.userRequest.indexOf('spawn-sync.js') === -1
    )
  })
  ...
  const outputPath = path.join(basedir, 'bin', 'lyra')
