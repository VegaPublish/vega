...
import {loadJson} from '@lyra/util/lib/safeJson'
...
const environment = lyraEnv ? lyraEnv : process.env.NODE_ENV
...
export default class CommandRunner {
  ...
  async runCommand(commandOrGroup, args, options) {
    if (!commandOrGroup) {
      this.handlers.outputter.print(
        generateCommandsDocumentation(this.commandGroups)
      )
    ...
    if (!commandInfo) {
      throw new Error(
        noSuchCommandText(commandOrGroup, subCommandName, this.commandGroups)
      )
    }
    ...
    const manifestPath = path.join(options.workDir, 'lyra.json')
    ...
    if (command.isGroupRoot) {
      return context.output.print(
        generateCommandsDocumentation(this.commandGroups, command.name)
      )
    }
    ...
    if (typeof command.action !== 'function') {
      ...
      debug(
        `Command "${cmdName}" doesnt have a valid "action"-property, showing help`
      )
      const groupName =
        command.group && command.group !== 'default' ? command.group : null
      return context.output.print(
        generateCommandDocumentation(command, groupName, subCommandName)
      )
    }
  ...
  resolveSubcommand(group, subCommandName, parentGroupName) {
    ...
    if (!subCommand) {
      throw new Error(
        noSuchCommandText(subCommandName, parentGroupName, this.commandGroups)
      )
    }
