...
const helpText = `
Run the command again within a Lyra project directory, where "@lyra/core"
...
export default (cmdName, parentGroupName, groups) => {
  ...
  if (isCoreCommand) {
    return `"${cmdName}" is not available outside of a Lyra project context.${helpText}`
  }
...
function suggestCommand(cmdName, group, parentGroupName = null) {
  ...
  const help = chalk.cyan('"lyra --help"')

  const didYouMean = suggestCmd ? `Did you mean ${suggest}? ` : ' '
  return parentGroupName
    ? `${input} is not a subcommand of "lyra ${parentGroupName}". ${didYouMean}See ${help}`
    : `${input} is not a lyra command. ${didYouMean}See ${help}`
}
