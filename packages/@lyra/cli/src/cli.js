...
import {resolveProjectRoot} from '@lyra/resolver'
...
const lyraEnv = process.env.LYRA_ENV || 'production' // eslint-disable-line no-process-env
...
module.exports = async function runCli(cliRoot) {
  ...
  // Translate `lyra -h <command>` to `lyra help <command>`
  if (core.h || core.help) {
...
function getCoreModulePath(workDir) {
  const pkgPath = resolveFrom.silent(workDir, '@lyra/core')
  if (pkgPath) {
    return pkgPath
  }

  const hasManifest = fse.existsSync(path.join(workDir, 'lyra.json'))
  if (hasManifest && process.argv.indexOf('install') === -1) {
    console.warn(
      chalk.yellow(
        [
          '@lyra/core not installed in current project',
          'Project-specific commands not available until you run `lyra install`'
        ].join('\n')
      )
    )
  }
...
function warnOnNonProductionEnvironment() {
  if (lyraEnv === 'production') {
    return
  }

  console.warn(
    chalk.yellow(
      knownEnvs.includes(lyraEnv)
        ? `[WARN] Running in ${lyraEnv} environment mode\n`
        : `[WARN] Running in ${chalk.red(
            'UNKNOWN'
          )} "${lyraEnv}" environment mode\n`
    )
  )
}
