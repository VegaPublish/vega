...
export default async (context, target, opts = {}) => {
  ...
  const lyraModules = filterLyraModules(getLocalManifest(context.workDir))
  ...
  const versions = await promiseProps(
    buildPackageArray(lyraModules, context.workDir, resolveOpts)
  )
...
function filterLyraModules(manifest) {
  ...
  const lyraDeps = Object.keys(dependencies)
    .filter(mod => mod.indexOf('@lyra/') === 0)
    .sort()

  return lyraDeps.reduce((versions, dependency) => {
