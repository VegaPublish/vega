...
const {pathTools} = require('@lyra/util')
...
module.exports = async (context, url) => {
  ...
  try {
    const projectManifest = await fse.readJson(path.join(workDir, 'lyra.json'))
  ...
  const tplVars = parseJson(manifestContent).lyraTemplate || {}
  const {minimumBaseVersion, minimumCliVersion} = tplVars

  if (minimumBaseVersion) {
    const installed = getLyraVersion(workDir)
    if (semver.lt(installed, minimumBaseVersion)) {
      throw new Error(
        `Template requires Lyra at version ${minimumBaseVersion}, installed is ${installed}`
      )
    }
  }
  ...
  if (minimumCliVersion && semver.lt(pkg.version, minimumCliVersion)) {
    throw new Error(
      `Template requires @lyra/cli at version ${minimumCliVersion}, installed is ${
        pkg.version
      }`
    )
  }
...
function getLyraVersion(workDir) {
  const basePkg = resolveFrom.silent(workDir, '@lyra/base/package.json')
