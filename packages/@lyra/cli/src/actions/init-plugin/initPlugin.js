import addPluginToManifest from '@lyra/util/lib/addPluginToManifest'
...
export default async function initPlugin(args, context, initOpts = {}) {
  ...
  output.print('This utility will walk you through creating a new Lyra plugin.')
  ...
  if (specifiedTemplateUrl) {
    specifiedTemplate = pluginTemplates.find(
      tpl => tpl.value === specifiedTemplateUrl
    )
  }
  ...
  const templateChoices = pluginTemplates.map(({value, name}) => ({
    value,
    name
  }))
...
async function bootstrapFromUrl(context, url) {
  ...
  const {
    ...
    inPluginsPath,
    dependencies
  } = await bootstrapFromTemplate(context, url)

  if (inPluginsPath) {
    const addIt = await prompt.single({
      type: 'confirm',
      message: 'Enable plugin in current Lyra installation?',
      default: true
    })

    if (addIt) {
      await addPluginToManifest(workDir, name.replace(/^lyra-plugin-/, ''))
    }
  }
  ...
  if (dependencies && inPluginsPath) {
    ...
    if (addDeps) {
      const deps = Object.keys(dependencies).map(
      ...
      output.print(
        'Remember to remove them from `package.json` if you no longer need them!'
      )
    }
  }
