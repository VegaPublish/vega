...
export default (args, context) => {
  ...
  if (!commandName) {
    debug('No command given to "help", showing generate Lyra CLI help')
    context.output.print(generateCommandsDocumentation(commandGroups))
    return
  }

  const defaultCommand = commandGroups.default.find(
  ...
  if (!subCommandName && !group) {
    debug(
      `No subcommand given, and we couldn't find a group with name "${group}"`
    )
  ...
  if (!subCommandName && group) {
    ...
    context.output.print(
      generateCommandsDocumentation(commandGroups, commandName)
    )
    return
  }
  ...
  if (subCommandName && !group) {
    debug(
      `Subcommand given, but couldn't find group with name "${commandName}"`
    )
    throw new Error(
      noSuchCommandText(subCommandName, commandName, commandGroups)
    )
  }

  const subCommand = context.commandRunner.resolveSubcommand(
    group,
    subCommandName,
    commandName
  )
  ...
  debug(
    'Subcommand "%s" for group "%s" found, showing help',
    subCommandName,
    commandName
  )
  context.output.print(
