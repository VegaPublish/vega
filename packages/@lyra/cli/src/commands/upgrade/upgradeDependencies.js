...
export default async (args, context) => {
  ...
  const needsUpdate =
    modules.length === 0
      ? allNeedsUpdate
      : allNeedsUpdate.filter(
          outOfDate => modules.indexOf(outOfDate.name) !== -1
        )
  ...
  if (needsUpdate.length === 0) {
    ...
    context.output.print(
      `${chalk.green('✔')} ${specified} Lyra modules are at latest versions`
    )
    return
  }
...
async function getModulesInNeedOfUpdate(context, target) {
  const versions = await findLyraModuleVersions(context, target, {
    includeCli: false
  })
