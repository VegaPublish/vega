...
export default {
  name: 'logout',
  signature: 'logout',
  description: 'Logs out of the Vegapublish.com session',
  async action(args, {output, prompt, apiClient}) {
    ...
    if (type === 'provisional') {
      ...
      output.print(
        chalk.yellow(
          "Logging out will make it super hard to claim your beautiful project :'("
        )
      )
    }
    ...
    async function logout() {
      ...
      if (token) {
        try {
          ...
          output.error(
            chalk.red(
              `Failed to communicate with the Lyra API:\n${err.message}`
            )
          )
          return
        }
