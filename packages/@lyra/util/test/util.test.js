...
import {reduceConfig, getLyraVersions, pathTools} from '../src'
...
describe('util', () => {
  describe('reduceConfig', () => {
    ...
    it('concats arrays', () => {
      const reduced = reduceConfig(
        {
          root: true,
          plugins: ['@lyra/base', '@lyra/components'],
      ...
      expect(reduced.plugins).to.deep.equal([
        '@lyra/base',
        '@lyra/components',
        'vision'
      ])
  ...
  describe('getLyraVersions', () => {
    it('extracts correct versions', () => {
      const versions = getLyraVersions(path.join(__dirname, 'versionsFixture'))
  ...
  describe('path tools', () => {
    ...
    it('can absolutify relative paths', () => {
      ...
      expect(absolutify('./util.test.js')).to.equal(
        path.join(process.cwd(), 'util.test.js')
      )
    })
