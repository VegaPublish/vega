...
import styles from './styles/LyraPanel.css'

addons.register('lyra/info', api => {
  addons.addPanel('lyra/info/panel', {
    title: 'Lyra',
    render: function render() {
      return <LyraPanel channel={addons.getChannel()} api={api} />
    }
  })
})

class LyraPanel extends React.PureComponent {
  constructor(props) {
    super(props)
  ...
  componentDidMount() {
    ...
    channel.on('lyra/info/set-info', this.onSetInfo)
  ...
  componentWillUnmount() {
    ...
    this.props.channel.removeListener('lyra/info/set-info', this.onSetInfo)
  }
  ...
  normalizePath(path) {
    ...
    const normalized =
      path.indexOf(basePath) === 0 ? path.slice(basePath.length) : path
    ...
    if (!part) {
      return (
        <div className={styles.noContent}>
          No Lyra info recorded for this component
        </div>
      )
    }
