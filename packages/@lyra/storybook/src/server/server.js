...
function startServer(msg) {
  webpackConfig.setLyraContext(msg.config)
  ...
  app.listen(port, msg.config.httpHost, error => {
    ...
    process.send({
      event: 'listening',
      url: `http://${msg.config.httpHost}:${port}/`
    })
  })
}
...
function unhandledMessage(msg) {
  console.error(
    `Unhandled message of type "${msg.event}"; %s`,
...
function tryLoadConfig(basePath) {
  try {
    return require(path.join(basePath, 'config', '@lyra', 'storybook.json'))
  } catch (err) {
    return {}
  }
}
