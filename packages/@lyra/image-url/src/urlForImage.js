...
export default function urlForImage(options) {
  ...
  crop.width = Math.round(
    asset.width - image.crop.right * asset.width - crop.left
  )
  crop.height = Math.round(
    asset.height - image.crop.bottom * asset.height - crop.top
  )
...
function specToImageUrl(spec) {
  const filename = `${spec.asset.id}-${spec.asset.width}x${spec.asset.height}.${
    spec.asset.format
  }`
  const baseUrl = `${spec.apiHost}/images/${spec.dataset}/${filename}`
  ...
  if (spec.rect) {
    ...
    if (isEffectiveCrop) {
      params.push(
        `rect=${spec.rect.left},${spec.rect.top},${spec.rect.width},${
          spec.rect.height
        }`
      )
    }
  ...
  if (spec.flipHorizontal || spec.flipVertical) {
    params.push(
      `flip=${spec.flipHorizontal ? 'h' : ''}${spec.flipVertical ? 'v' : ''}`
    )
  }
