...
const resolver = require('@lyra/resolver')
const util = require('@lyra/util')

const reduceConfig = util.reduceConfig
const getLyraVersions = util.getLyraVersions
...
function registerLoader(options) {
  ...
  parts.implementations = Object.keys(parts.implementations).reduce(
  ...
  Module._resolveFilename = (request, parent) => {
    // `lyra:debug` returns the whole resolve result
    if (request === 'lyra:debug') {
    ...
    if (configMatch) {
      ...
      if (configFor === 'lyra') {
        const lyraConfig = require(path.join(basePath, 'lyra.json'))
        require.cache[request] = getModule(
          request,
          reduceConfig(lyraConfig, env)
        )
        return request
      }
  ...
  if (options.stubCss) {
    ...
    const postcss = require('@lyra/webpack-integration/v3')
    cssHook({
      generateScopedName:
        options.generateScopedName || '[name]__[local]___[hash:base64:5]',
      prepend: postcss
