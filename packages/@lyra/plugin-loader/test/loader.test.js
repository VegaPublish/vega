...
test('should be able to include the lyra debug part', t => {
  pluginLoader({basePath: path.join(__dirname, 'fixture')})
  const debug = require('lyra:debug')
...
test('should be able to load lyra instance config', t => {
  pluginLoader({basePath: path.join(__dirname, 'fixture')})

  const config = require('config:lyra')
...
test('should be able to load config for a namespaced plugin', t => {
  ...
  const config = require('config:@lyra/some-plugin')
  t.is(config.subway, 'coding')
})

test('should be able to load lyra plugin versions', t => {
  pluginLoader({basePath: path.join(__dirname, 'versionsFixture')})

  const versions = require('lyra:versions')
  t.deepEqual(versions, {
    '@lyra/base': '0.999.99',
    '@lyra/components': '0.777.77'
  })
})
...
;((/*noop*/) => {})('should be able stub CSS loading', t => {
...
test('should be able to override config parts', t => {
  const overrides = {'config:lyra': [{api: {apiHost: 'heisann'}}]}
  pluginLoader({overrides})

  const config = require('config:lyra')
  t.is(config.api.apiHost, 'heisann')
})
