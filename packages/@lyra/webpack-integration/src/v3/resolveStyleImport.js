...
const lyraCache = new PAC({
  load: resolveLyraImport,
  maxAge: 1000
})
...
function resolveLyraImport(id, basePath) {
...
function getStyleResolver(opts) {
  return function resolveStyleProxy(moduleId, basedir) {
    ...
    const resolveStyleImport = isLyraPart(id)
      ? lyraCache.get(id, opts.from)
