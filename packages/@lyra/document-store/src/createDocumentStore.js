...
import {BufferedDocument, Mutation} from '@lyra/mutator'

const NOOP = () => {}

function createBufferedDocument(documentId, serverEvents$, doCommit) {
  const reconnects$ = serverEvents$.pipe(
      ...
      return {
        ...
        createIfNotExists(document) {
          bufferedDocument.add(
            new Mutation({mutations: [{createIfNotExists: document}]})
          )
        },
        createOrReplace(document) {
          bufferedDocument.add(
            new Mutation({mutations: [{createOrReplace: document}]})
          )
        },
        delete() {
          bufferedDocument.add(
            new Mutation({mutations: [{delete: {id: documentId}}]})
          )
        },
        commit() {
  ...
  return {
    ...
    createIfNotExists(document) {
      cachedBuffered.subscribe(bufferedDoc =>
        bufferedDoc.createIfNotExists(document)
      )
    },
    createOrReplace(document) {
      cachedBuffered.subscribe(bufferedDoc =>
        bufferedDoc.createOrReplace(document)
      )
    },
    delete() {
...
module.exports = function createDocumentStore({serverConnection}) {
  ...
  function checkoutPair(idPair) {
    ...
    const serverEvents$ = serverConnection
      .byIdPair({publishedId, draftId})
      .pipe(share())
  ...
  function byIds(documentIds) {
    return new Observable(observer => {
      const documentSubscriptions = documentIds.map(id =>
        byId(id).subscribe(observer)
      )
