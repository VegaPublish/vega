...
function fieldHasReferenceTo(fieldDef, refType) {
  return arrify(fieldDef.to || []).some(
    memberTypeDef => memberTypeDef.type === refType
  )
}

function isImageAssetField(fieldDef) {
  return fieldHasReferenceTo(fieldDef, 'lyra.imageAsset')
}
...
function isFileAssetField(fieldDef) {
  return fieldHasReferenceTo(fieldDef, 'lyra.fileAsset')
}
...
export default function guessPreviewFields(rawObjectTypeDef) {
  ...
  let titleField = TITLE_CANDIDATES.find(candidate =>
    stringFieldNames.includes(candidate)
  )

  let descField = DESCRIPTION_CANDIDATES.find(
    candidate =>
      candidate !== titleField && stringFieldNames.includes(candidate)
  )
  ...
  const mediaField = (rawObjectTypeDef.fields || []).find(
  ...
  const select = omitBy(
    {
      ...
      imageUrl:
        !mediaField && imageAssetPath ? `${imageAssetPath}.url` : undefined,
