...
export default (config = {}) => {
  ...
  if (!reactPath || !reactDomPath) {
    const missingErr = [
      ...
      'These need to be declared in `package.json` and be installed for Lyra to work'
    ].join('\n')
  ...
  return {
    ...
    module: {
      rules: [
        {
          test: /\.jsx?/,
          exclude: /((packages\/@lyra)|(packages\/@vega)|node_modules|bower_components)/,
          use: {
