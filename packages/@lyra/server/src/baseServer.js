...
import {resolveParts} from '@lyra/resolver'
import getStaticBasePath from './util/getStaticBasePath'

const docPart = 'part:@lyra/base/document'
const initPart = 'part:@lyra/server/initializer'
...
const getTitle = (project = {}) => {
  ...
  return projectName ? `${projectName} - Lyra` : 'Lyra'
}
...
const getDocumentComponent = basePath =>
  resolveParts({basePath}).then(res => {
    ...
    if (!part) {
      throw new Error(
        `Part '${docPart}' is not implemented by any plugins, are you missing @lyra/base?`
      )
    }
      ...
      Object.assign(
        {
          // URL base path
          basePath:
            process.env.STUDIO_BASEPATH || (project && project.basePath) || '',
          title: getTitle(project),
          stylesheets: ['css/main.css'].map(item =>
