...
import {checkoutPair, FormBuilder} from 'part:@lyra/form-builder'
...
import {getDraftId, getPublishedId} from 'part:@lyra/base/util/draft-utils'
...
function isRecoverable(draft, published) {
  return (
    !exists(draft, published) &&
    (draft.deletedSnapshot || published.deletedSnapshot)
  )
}
...
export default class EditorWrapper extends React.Component {
  ...
  setup(documentId) {
    ...
    this.draft = draft
    this.validateLatestDocument = debounce(
      promiseLatest(this.validateDocument, 300)
    )

    const published$ = this.published.events
    const draft$ = this.draft.events

    this.subscription = merge(
      published$.pipe(map(event => ({...event, version: 'published'}))),
      draft$.pipe(map(event => ({...event, version: 'draft'})))
    )
      .pipe(tap(this.receivePatchEvent))
      .subscribe(event => {
  ...
  validateDocument = async () => {
    ...
    if (!type) {
      // eslint-disable-next-line no-console
      console.warn(
        'Schema for document type "%s" not found, skipping validation'
      )
      return []
    }
