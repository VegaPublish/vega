...
import Dialog from 'part:@lyra/components/dialogs/fullscreen'
  ...
  class ConfirmUnpublish extends React.PureComponent {
    ...
    render() {
      ...
      const actions = [
        canContinue && {
          name: 'confirm',
          title: hasReferringDocuments
            ? 'Try to unpublish anyway'
            : 'Unpublish now'
        },
        {name: 'cancel', title: 'Cancel', kind: 'secondary'}
      ].filter(Boolean)

      const title = isCheckingReferringDocuments
        ? 'Checking…'
        : 'Confirm unpublish'
          ...
          {!isCheckingReferringDocuments &&
            !hasReferringDocuments && (
              <div style={{padding: 10}}>
                ...
                <h2>Careful!</h2>
                <p>
                  If you unpublish, this document will no longer be available
                  for the public, but it will not be deleted and can be
                  published again later if you change your mind.
                </p>
              </div>
            )}
        </Dialog>
