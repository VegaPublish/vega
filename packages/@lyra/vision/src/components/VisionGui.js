...
class VisionGui extends React.PureComponent {
  ...
  handleListenExecution() {
    ...
    const url = client.getUrl(
      client.getDataUrl('listen', encodeQueryString(query, params))
    )
  ...
  handleQueryExecution() {
    ...
    const url = client.getUrl(
      client.getDataUrl('query', encodeQueryString(query, params))
    )
    const queryStart = Date.now()

    this.subscribers.query = client
  ...
  render() {
    ...
    const hasResult =
      !error && !queryInProgress && typeof result !== 'undefined'
    ...
    const visionClass = ['lyra-vision', this.context.styles.visionGui.root]
      .filter(Boolean)
      .join(' ')
    const headerClass = ['lyra-vision', this.context.styles.visionGui.header]
      .filter(Boolean)
      .join(' ')
    return (
        ...
        <div className={styles.splitContainer}>
          <SplitPane split="vertical" minSize={150} defaultSize={400}>
            ...
            <div className={styles.resultContainer}>
              <h3 className={styles.inputLabelQuery || 'resultLabel'}>
                Result
              </h3>
