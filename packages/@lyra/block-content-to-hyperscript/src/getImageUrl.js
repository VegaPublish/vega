...
const urlBuilder = require('@lyra/image-url')
const objectAssign = require('object-assign')

const enc = encodeURIComponent
const materializeError = `You must either:
  - Pass \`apiHost\` and \`dataset\` to the block renderer
...
const buildUrl = props => {
  ...
  return urlBuilder(objectAssign({apiHost, dataset}, options.imageOptions || {}))
    .image(node)
    .toString()
}
