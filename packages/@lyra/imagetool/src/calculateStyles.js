...
export default function calculateStyles(options = {}) {
  ...
  const containerAspect =
    readAspectRatio(options.container) || imageAspect * readCropAspect(crop)
...
function readAspectRatio(opts) {
  ...
  if (opts.hasOwnProperty('height') || opts.hasOwnProperty('width')) {
    if (typeof opts.height !== 'number' && typeof opts.width !== 'number') {
      throw new Error(
        `Height and width must be numbers, got ${JSON.stringify(opts)}`
      )
    }
...
function calculateHotSpotCrop(sourceAspect, descriptor, spec) {
  ...
  const maxHotspotYScale =
    ((1.0 / hotspot.height) * cropAspect) / viewportAspect
  ...
  if (minFullBleedScale > maxScale) {
    ...
    const hotspotLeft =
      hotspot.x * outCrop.width - (hotspot.width * outCrop.width) / 2
    switch (alignment.x) {
      case 'left':
    ...
    const hotspotTop =
      hotspot.y * outCrop.height - (hotspot.height * outCrop.height) / 2
    switch (alignment.y) {
      case 'top':
      ...
      default:
        throw new Error(
          `Invalid y alignment: '${
            alignment.y
          }'. Must be either 'top', 'bottom' or 'center'`
        )
    }
