...
export default class ImageTool extends React.PureComponent {
  ...
  getCropRect() {
    ...
    return new Rect()
      ...
      .shrink(MARGIN_PX * this.getScale())
      .cropRelative(
        Rect.fromEdges(value.crop || DEFAULT_CROP).clamp(new Rect(0, 0, 1, 1))
      )
  }
  ...
  emitResize(pos) {
    ...
    onChange(
      this.applyHotspotResizeBy(value, {height: delta.y, width: delta.x})
    )
  }
  ...
  getClampedValue() {
    ...
    const crop = Rect.fromEdges(value.crop || DEFAULT_CROP).clamp(
      new Rect(0, 0, 1, 1)
    )
  ...
  getCursor() {
    ...
    const activeCropArea =
      this.state.cropping || this.getActiveCropHandleFor(mousePosition)
    if (activeCropArea) {
      return getCropCursorForHandle(activeCropArea)
    }

    const mouseOverDragHandle = utils2d.isPointInCircle(
      mousePosition,
    ...
    const mouseoverHotspot = utils2d.isPointInEllipse(
      mousePosition,
      this.getHotspotRect()
    )
    const mouseoverCropRect = utils2d.isPointInRect(
      mousePosition,
      this.getCropRect()
    )
