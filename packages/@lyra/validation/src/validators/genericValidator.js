...
const type = (expected, value, message) => {
  ...
  if (actualType !== expected && actualType !== 'undefined') {
    return new ValidationError(
      message || `Expected type "${expected}", got "${actualType}"`
    )
  }
...
const all = (children, value, message) =>
  multiple(children, value).then(results => {
    ...
    return numErrors === 0
      ? true
      : formatValidationErrors(message, results, {
          separator: ' - AND - ',
          operator: 'AND'
        })
  })
...
const valid = (allowedValues, actual, message) => {
  ...
  const value =
    (valueType === 'number' || valueType === 'string') && `${actual}`
...
function formatValidationErrors(message, results, options = {}) {
  ...
  return results.length === 1
    ...
    : new ValidationError(
        message ||
          `[${results.map(err => err.item.message).join(options.separator)}]`,
        errOpts
      )
}
