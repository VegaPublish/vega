...
module.exports = (rule, value, options = {}) => {
  ...
  function validateRule(curr) {
    if (typeof curr.flag === 'undefined') {
      return Promise.reject(
        new Error('Invalid rule, did not contain "flag"-property')
      )
    }
    ...
    if (!validator) {
      ...
      return Promise.reject(
        new Error(`Validator for flag "${curr.flag}" not found for ${forType}`)
      )
    }
    ...
    if (itemConstraint && itemConstraint.type === rule.FIELD_REF) {
      if (!options.parent) {
        return Promise.reject(
          new Error('Field reference provided, but no parent received')
        )
      }
  ...
  function processResult(result) {
    ...
    return results.concat(
      result.paths.map(path => ({path, level: rule._level, item: result}))
    )
  }
}
