...
describe('child rules', () => {
  ...
  test('all() rules - multiple failures', async () => {
    ...
    await expect(rule.validate('Lyra constellation')).resolves.toMatchSnapshot(
      'all() rules - match'
    )
    await expect(rule.validate('moop')).resolves.toMatchSnapshot(
  ...
  test('all() rules - single failure, custom, common error', async () => {
    const rule = Rule.string()
      ...
      .error(
        'Needs to start with a capital letter and then follow with lowercase characters'
      )
  ...
  test('all() rules - single failure, custom, common error', async () => {
    const rule = Rule.string()
      ...
      .error(
        'Needs to be a capital letter followed by at least 4 lowercase characters'
      )
  ...
  test('either() rules - all matches', async () => {
    ...
    await expect(rule.validate('Red')).resolves.toMatchSnapshot(
      'either() rules - all match'
    )
    await expect(rule.validate('nope')).resolves.toMatchSnapshot(
