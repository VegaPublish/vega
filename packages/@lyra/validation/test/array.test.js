...
describe('array', () => {
  ...
  test('min length constraint', async () => {
    ...
    await expect(rule.validate(['a', 'b', 'c'])).resolves.toMatchSnapshot(
      'min length: valid'
    )
  })
  ...
  test('exact length constraint', async () => {
    ...
    await expect(rule.validate(['a', 'b'])).resolves.toMatchSnapshot(
      'exact length: valid'
    )
  })
  ...
  test('unique constraint (default, object values)', async () => {
    ...
    await expect(
      rule.validate(['a', 'b', 'c', 'd'].map(ref))
    ).resolves.toMatchSnapshot('object unique: valid')
    await expect(
      rule.validate(['a', 'b', 'c', 'a'].map(ref))
    ).resolves.toMatchSnapshot('object unique: duplicates')
  })
  ...
  test('unique constraint (default, array values)', async () => {
    ...
    await expect(
      rule.validate(['a', 'b', 'c', 'd'].map(refArr))
    ).resolves.toMatchSnapshot('array unique: valid')
    await expect(
      rule.validate(['a', 'a', 'c', 'd'].map(refArr))
    ).resolves.toMatchSnapshot('array unique: duplicates')
  })

  test('unique constraint (default, bool values)', async () => {
    const rule = Rule.array().unique()
    await expect(rule.validate([true, false])).resolves.toMatchSnapshot(
  ...
  test('unique constraint (default, numeric values)', async () => {
    const rule = Rule.array().unique()
    await expect(rule.validate([1, 3])).resolves.toMatchSnapshot(
      'numeric unique: valid'
    )
    await expect(rule.validate([3, 1, 3])).resolves.toMatchSnapshot(
