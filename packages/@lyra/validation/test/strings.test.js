...
describe('string', () => {
  test('required constraint', async () => {
    ...
    await expect(rule.validate('')).resolves.toMatchSnapshot(
    ...
    await expect(rule.validate('abc')).resolves.toMatchSnapshot(
      'required: valid'
    )
  })
  ...
  test('min length constraint', async () => {
    ...
    await expect(rule.validate('a')).resolves.toMatchSnapshot(
  ...
  test('max length constraint', async () => {
    ...
    await expect(rule.validate('abcdefg')).resolves.toMatchSnapshot(
      'max length: too long'
    )
    await expect(rule.validate('abc')).resolves.toMatchSnapshot(
      'max length: valid'
  ...
  test('exact length constraint', async () => {
    ...
    await expect(rule.validate('abcdefgh')).resolves.toMatchSnapshot(
      'exact length: too long'
    )
    await expect(rule.validate('abc')).resolves.toMatchSnapshot(
      'exact length: too short'
    )
    await expect(rule.validate('abcde')).resolves.toMatchSnapshot(
      'exact length: valid'
    )
  })
  ...
  test('uppercase constraint', async () => {
    ...
    await expect(rule.validate('lyra')).resolves.toMatchSnapshot(
    ...
    await expect(rule.validate('SäNITY')).resolves.toMatchSnapshot(
  ...
  test('regex constraint', async () => {
    ...
    await expect(rule.validate('Lyra')).resolves.toMatchSnapshot('regex: match')
  })
  ...
  test('uri constraint', async () => {
    ...
    await expect(
      rule.validate('https://vegapublish.com/')
    ).resolves.toHaveLength(0)
  })

  test('uri constraint (with unicode chars)', async () => {
    const rule = Rule.string().uri()
    await expect(rule.validate('Blåbærsyltetøy')).resolves.toMatchSnapshot(
      'uri: non-match'
    )
    await expect(
      rule.validate('https://en.wikipedia.org/wiki/San_&_Søn')
    ).resolves.toHaveLength(0)
    await expect(
      rule.validate('https://zh.wikipedia.org/wiki/心形符號')
    ).resolves.toHaveLength(0)
    await expect(
      rule.validate('https://ru.wikipedia.org/wiki/Зонтичные')
    ).resolves.toHaveLength(0)
    await expect(rule.validate('https://påtapp.no/oslo')).resolves.toHaveLength(
      0
    )
  })
  ...
  test('uri constraint (invalid protocol)', async () => {
    ...
    await expect(
      rule.validate('https://vegapublish.com/')
    ).resolves.toMatchSnapshot('uri: protocol non-match')
    await expect(
      rule.validate('ftp://code.vegapublish.com/')
    ).resolves.toHaveLength(0)
  })
  ...
  test('uri constraint (credentials)', async () => {
    ...
    await expect(
      rule.validate('http://foo:bar@vegapublish.com/')
    ).resolves.toHaveLength(0)
    await expect(
      rule.validate('http://vegapublish.com/')
    ).resolves.toHaveLength(0)
    ...
    await expect(
      rule.validate('http://foo:bar@vegapublish.com/')
    ).resolves.toMatchSnapshot('uri: credentials specified but not allowed')
    await expect(
      rule.validate('http://espen@vegapublish.com/')
    ).resolves.toMatchSnapshot('uri: username specified but not allowed')
  })
  ...
  test('custom rule with string', async () => {
    ...
    await expect(rule.validate('hei')).resolves.toMatchSnapshot(
      'not a palindrome'
    )
