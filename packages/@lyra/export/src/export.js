...
function exportDataset(opts) {
  ...
  debug(
    'Outputting to %s',
    options.outputPath === '-' ? 'stdout' : options.outputPath
  )
  const outputStream =
    options.outputPath === '-'
      ? process.stdout
      : fse.createWriteStream(options.outputPath)

  let assetStreamHandler = assetHandler.noop
  if (!options.raw) {
    assetStreamHandler = options.assets
      ? assetHandler.rewriteAssets
      : assetHandler.stripAssets
  }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const inputStream = await getDocumentsStream(
      options.client,
      options.dataset
    )
