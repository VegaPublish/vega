...
class AssetHandler {
  ...
  queueAssetDownload(assetDoc, dstPath, type) {
    if (!assetDoc.url) {
      debug(
        'Asset document "%s" does not have a URL property, skipping',
        assetDoc._id
      )
      return
    }
    ...
    debug(
      'Adding download task for %s (destination: %s)',
      assetDoc._id,
      dstPath
    )
    this.queueSize++
  ...
  async downloadAsset(assetDoc, dstPath) {
    ...
    if (stream.statusCode !== 200) {
      ...
      this.reject(
        new Error(
          `Referenced asset URL "${url}" returned HTTP ${stream.statusCode}`
        )
      )
      return
    }
    ...
    const hash = await writeHashedStream(
      path.join(this.tmpDir, dstPath),
      stream
    )
  ...
  findAndModify = async (item, action) => {
    if (Array.isArray(item)) {
      const children = await Promise.all(
        item.map(child => this.findAndModify(child, action))
      )
    ...
    if (isAsset && action === ACTION_REWRITE) {
      ...
      if (isModernAsset(assetId)) {
        ...
        return {_lyraAsset: `${assetType}@file://./${filePath}`, ...other}
      }

      // Legacy asset
      const type =
        this.assetsSeen.get(assetId) || (await this.lookupAssetType(assetId))
      const filePath = `${type}s/${generateFilename(assetId)}`
      return {_lyraAsset: `${type}@file://./${filePath}`, ...other}
    }
  ...
  lookupAssetType = async assetId => {
    const docType = await this.client.fetch('*[_id == $id][0]._type', {
      id: assetId
    })
