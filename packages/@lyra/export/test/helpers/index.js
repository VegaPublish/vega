...
const getMockClient = () => ({
  config: () => ({dataset: '__test__'}),
  fetch: (query, params) =>
    query.endsWith('._type')
      ? `lyra.imageAsset`
      : `http://localhost:32323/${params.id}.jpg`
})
...
const arrayToStream = docs =>
  stringToStream(docs.map(doc => JSON.stringify(doc)).join('\n'))
