...
if (showHelp) {
  console.log('Usage: lyra-check [DIRECTORY]')
...
if (showVersion) {
  console.log(`lyra-check ${pkg.version}`)
  process.exit()
}
...
fse
  ...
  .catch(err => {
    console.error(
      chalk.red(`${tag} Failed to read "${manifestDir}":\n${err.message}`)
    )

    process.exit(1)
  })
  .then(() => lyraCheck({dir, productionMode}))
  .catch(err => {
    console.error(
      chalk.red(err.lyraCheck ? `${tag}\n\n${err.message}` : err.stack)
    )
