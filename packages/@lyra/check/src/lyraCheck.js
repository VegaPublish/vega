...
const resolveParts = require('@lyra/resolver').resolveParts

const includes = (arr, val) => arr.indexOf(val) !== -1

function lyraCheck(options) {
  return resolveParts({
...
function verifyImplementationExists(impl, parentDirContent) {
  ...
  if (found) {
    return new Error(
      `Part "${impl.partName}" was attempted to be implemented by "${
        impl.path
      }",` +
        `but the file is actually located at "${path.join(
          impl.dirName,
          found
        )}" -` +
        ' Lyra uses case-sensitive file names.'
    )
  }
...
function checkImplementationMsg(impl) {
  const location =
    impl.plugin === '(project root)'
      ? 'Check "lyra.json"'
      : `Check "${impl.plugin}"`

  return `${location} and keep in mind that paths in Lyra are case-sensitive.`
}
...
function getProductionHint() {
  return (
    '[NOTE]: lyra-check is running in production mode - ' +
    'perhaps you have defined a `compiled` path in `lyra.json`? ' +
    'This tells Lyra to look for the files in a different location ' +
