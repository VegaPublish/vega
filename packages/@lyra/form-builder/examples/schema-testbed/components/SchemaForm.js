...
const schema =
  params.schemaName &&
  params.typeName &&
  Schema.compile(sourceSchemas[params.schemaName])
...
export default class Main extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevState.inspect !== this.state.inspect) {
      document.body.style.overflow =
        this.state.inspect === 'fullscreen' ? 'hidden' : ''
    }
  }
  ...
  renderInspect() {
    ...
    return (
      <div
        className={
          styles[inspect === 'docked' ? 'inspectPane' : 'inspectPaneFullScreen']
        }
      >
        <button
