...
export default class OptionsArrayInput extends React.PureComponent {
  ...
  handleChange = (isChecked, optionValue) => {
    ...
    this.props.onChange(
      PatchEvent.from(nextValue.length > 0 ? set(nextValue) : unset())
    )
  }
  ...
  getMemberTypeOfItem(option) {
    ...
    return type.of.find(
      memberType =>
        memberType.name ===
        resolveTypeName(resolveValueWithLegacyOptionsSupport(option))
    )
  }
    ...
    return (
      ...
      >
        {options.map((option, index) => {
          ...
          if (!optionType) {
            const actualType = resolveTypeName(
              resolveValueWithLegacyOptionsSupport(option)
            )
            const validTypes = type.of.map(ofType => ofType.name)
            return (
              <div key={option._key || index} className={styles.error}>
                Invalid option type: Type <code>{actualType}</code> not valid
                for array of [{validTypes.join(', ')}
                ]. Check the list options of this field
              </div>
            )
          }
          const checked = inArray(
            value,
            resolveValueWithLegacyOptionsSupport(option)
          )
          return (
