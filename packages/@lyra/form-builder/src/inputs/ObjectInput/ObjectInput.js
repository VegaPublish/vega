...
function getCollapsedWithDefaults(options = {}, level) {
  ...
  //       --> relevant: https://github.com/lyra-io/lyra/issues/537
...
export default class ObjectInput extends React.PureComponent {
  ...
  handleFieldChange = (fieldEvent: PatchEvent, field) => {
    ...
    if (!isRoot) {
      event = event.prepend(
        setIfMissing(type.name === 'object' ? {} : {_type: type.name})
      )
      if (value) {
  ...
  renderFieldset(fieldset, fieldsetIndex) {
    ...
    const isExpanded =
      focusPath.length > 0 &&
      fieldset.fields.some(field => focusPath[0] === field.name)
  ...
  getRenderedFields() {
    ...
    if (!type.fieldsets) {
      ...
      return (type.fields || []).map((field, i) =>
        this.renderField(field, level + 1, i)
      )
    }
