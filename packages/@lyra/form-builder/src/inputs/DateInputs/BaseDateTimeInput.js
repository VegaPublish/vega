...
import CalendarIcon from 'part:@lyra/base/calendar-icon'
...
export default class BaseDateTimeInput extends React.Component<Props, State> {
  ...
  render() {
    ...
    const placeholder =
      this.props.placeholder || `e.g. ${moment().format(format)}`
        ...
        {isDialogOpen && (
          ...
          >
            <div
              className={
                dateOnly
                  ? styles.dialogDatePicker
                  : styles.dialogDatePickerWithTime
              }
            >
              <DatePicker
                inline
