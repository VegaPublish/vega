...
import {editorValueToBlocks, normalizeBlock} from '@lyra/block-tools'
...
function setNodePatch(
  ...
  return set(normalizeBlock(changedBlock), [
    {_key: blocks[operation.path[0]]._key}
  ])
}
...
function insertNodePatch(
  ...
  if (operation.path.length === 1) {
    ...
    const oldData = change.value.document.nodes
      .get(operation.path[0])
      .data.toObject()
...
function removeNodePatch(
  ...
  if (patches.length === 0) {
    throw new Error(
      `Don't know how to unset ${JSON.stringify(
        operation.toJSON(VALUE_TO_JSON_OPTS)
      )}`
    )
  }
  return patches
