...
import {editorValueToBlocks, blocksToEditorValue} from '@lyra/block-tools'
...
export function toggleListItem(change, listItemName) {
  ...
  const active = blocks.some(
    block => block.data.get('listItem') === listItemName
  )
...
export function createFormBuilderSpan(
  change,
  annotationName,
  key,
  originalSelection
...
export function insertInlineObject(change, objectType, blockContentType) {
  ...
  const inlineObject = newBlock.nodes.find(
    node =>
      node.data &&
      node.data.get('value') &&
      node.data.get('value')._oldKey === key
  )
