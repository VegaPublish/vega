...
function findEditNode(focusPath, editorValue) {
  ...
  const focusInlineKey =
    ...
    focusPath[2]._key
  const markDefKey =
    focusPath[2] && focusPath[1] === 'markDefs' && focusPath[2]._key
  let key
  if (markDefKey) {
...
export default class BlockEditor extends React.PureComponent<Props> {
  ...
  renderNodeEditor() {
    ...
    if (slateNode.type === 'span') {
      ...
      type = blockContentFeatures.annotations.find(
        an => an.value === focusedAnnotationName
      ).type
  ...
  checkScrollHeight = () => {
    if (this._scrollContainer && this._editorWrapper) {
      this.setState({
        preventScroll:
          this._scrollContainer.offsetHeight < this._editorWrapper.offsetHeight
      })
    }
  }
  ...
  render() {
    ...
    return (
      <div className={styles.root} ref={this.setRootElement}>
        ...
        {!fullscreen && (
          <div className={isFocused ? styles.focus : ''}>
            {this.renderEditor()}
          </div>
        )}
