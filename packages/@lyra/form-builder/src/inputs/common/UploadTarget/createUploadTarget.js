...
function convertImagesToFilesAndClearContentEditable(
  ...
  return new Promise(resolve => setTimeout(resolve, 10)) // add a delay so the paste event can finish
    ...
    .then(imageBlobs =>
      imageBlobs.map(
        blob => new File([blob], 'pasted-image.jpg', {type: targetFormat})
      )
    )
}
// needed by Edge
  ...
  return class UploadTargetFieldset extends React.Component<Props, State> {
      ...
      extractPastedFiles(event.clipboardData)
        .then(files => {
          return files.length > 0
            ? files
            : // Invoke Safari hack
              convertImagesToFilesAndClearContentEditable(
                this._pasteInput,
                'image/jpeg'
              )
        })
    ...
    uploadFiles(files: Array<File>) {
      ...
      const rejected = tasks.filter(
        task => task.uploaderCandidates.length === 0
      )
            ...
            >
              {ambiguous.map(task => (
                <div key={task.file.name}>
                  The file {task.file.name} can be converted to several types of
                  content. Please select how you want to represent it:
                  <ul>
