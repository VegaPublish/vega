...
import * as sagaPatchAdapter from './utils/sagaPatchAdapter'
...
function preparePatchEvent(event) {
  ...
  return {
    ...event,
    patches: sagaPatchAdapter.toFormBuilder(event.origin, patches)
  }
}
...
function wrap(document) {
  const events$ = document.events.pipe(
    map(
      event => (event.type === 'mutation' ? preparePatchEvent(event) : event)
    ),
  ...
  return {
    ...
    patch(patches: Array<Patch>) {
      document.patch(sagaPatchAdapter.toSaga(patches))
    }
  }
}
