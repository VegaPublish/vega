...
import {observeForPreview} from 'part:@lyra/base/preview'
...
function buildConstraintFromType(type, terms) {
  ...
  const stringFieldConstraints = flatten(
    stringFieldPaths.map(fieldPath =>
      terms.map(term => `${fieldPath} match '${term}*'`)
    )
  )
...
export function search(textTerm, referenceType) {
  ...
  const typeConstraints = referenceType.to.map(type =>
    buildConstraintFromType(type, terms)
  )
