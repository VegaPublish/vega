...
import {blocksToEditorValue, editorValueToBlocks} from '@lyra/block-tools'
...
jest.mock('part:@lyra/base/client', () => null, {virtual: true})

const blockContentType = blocksSchema
  .get('blogPost')
  ...
  tests.forEach(test => {
    ...
    it(test, () => {
      ...
      const operations = new List(
        JSON.parse(fs.readFileSync(path.resolve(dir, 'operations.json'))).map(
