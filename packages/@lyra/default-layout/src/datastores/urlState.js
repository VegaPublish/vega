...
function resolveUrlStateWithDefaultSpace(state) {
  ...
  const defaultSpace =
    CONFIGURED_SPACES.find(ds => ds.default) || CONFIGURED_SPACES[0]
...
function resolveIntentState(currentState, intentState) {
  ...
  const matchingTool = (currentTool ? [currentTool, ...tools] : tools).find(
    tool =>
      ...
      tool.canHandleIntent(intent, params)
  )

  if (matchingTool) {
    const toolState = matchingTool.getIntentState(intent, params)
    const currentWithState =
      resolveUrlStateWithDefaultSpace(currentState) || currentState
