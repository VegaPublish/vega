...
import styles from 'part:@lyra/components/selects/style-style'
import {uniqueId, includes} from 'lodash'
import FaAngleDown from 'part:@lyra/base/angle-down-icon'
import CircleThinIcon from 'part:@lyra/base/circle-thin-icon'
import CircleCheckIcon from 'part:@lyra/base/circle-check-icon'
...
import {List} from 'part:@lyra/components/lists/default'
...
class StyleSelect extends React.PureComponent {
  ...
  componentDidMount() {
    this._keyUpHandler = document.body.addEventListener(
      'keyup',
      this.handleKeyUp
    )
    this._keyDownHandler = document.body.addEventListener(
      'keydown',
      this.handleKeyDown
    )
  }
  ...
  handleKeyDown = event => {
    ...
    if (items) {
      ...
      if (
        showList &&
        event.key == 'ArrowDown' &&
        arrowNavigationPosition < items.length - 1
      ) {
        event.preventDefault()
        this.setState({
          arrowNavigationPosition: arrowNavigationPosition + 1
                    ...
                    <div className={styles.portal}>
                      <Popper placement="bottom-start">
                        ...
                        <CaptureOutsideClicks
                          onClickOutside={
                            isActive ? this.handleCloseList : undefined
                          }
                        >
                            ...
                            <List className={styles.list}>
                              {items.map((item, index) => {
                                ...
                                const isItemActive =
                                  ...
                                  value[0] == item
                                const isSelected =
                                  index === this.state.arrowNavigationPosition
                                const classNames = [
                                  isItemActive
                                    ? styles.itemActive
                                    : styles.item,
