...
Open a query that listens for updates on matched documents, using the given parameters (if any). The return value is an [RxJS Observable](https://github.com/reactivex/rxjs). When calling `subscribe()` on the observable, a subscription is returned which can be used to unsubscribe from the query.
...
👉 Read more about [assets in Lyra](https://vegapublish.com/docs/http-api/assets)
