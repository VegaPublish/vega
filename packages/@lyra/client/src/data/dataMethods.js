...
module.exports = {
  ...
  fetch(query, params, options = {}) {
    const mapResponse =
      options.filterResponse === false ? res => res : res => res.result

    const observable = this._dataRequest('query', {query, params}).pipe(
      map(mapResponse)
    )
  ...
  _dataRequest(endpoint, body, options = {}) {
    ...
    return this._requestObservable(reqOptions).pipe(
      ...
      map(res => {
        ...
        const ids = returnFirst
          ? results[0] && results[0].id
          : results.map(mut => mut.id)
        return {
