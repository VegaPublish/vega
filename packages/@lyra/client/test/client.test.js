...
const lyraClient = require('../src/lyraClient')

const LyraClient = lyraClient
...
const apiHost = 'api.lyra.url'
const projectHost = () => `https://${apiHost}`
const clientConfig = {
  apiHost: `https://${apiHost}`,
  dataset: 'foo'
}

const getClient = conf => lyraClient(assign({}, clientConfig, conf || {}))
...
test('can construct client with new keyword', t => {
  const client = new LyraClient({dataset: 'abc123'})
  t.equal(client.config().dataset, 'abc123', 'constructor opts are set')
  t.end()
})

test('can construct client without new keyword', t => {
  const client = lyraClient({dataset: 'abc123'})
  t.equal(client.config().dataset, 'abc123', 'constructor opts are set')
  t.end()
})
...
test('can get and set config', t => {
  const client = lyraClient({dataset: 'abc123'})
  t.equal(client.config().dataset, 'abc123', 'constructor opts are set')
  t.equal(client.config({dataset: 'def456'}), client, 'returns client on set')
  t.equal(client.config().dataset, 'def456', 'new config is set')
  t.end()
})
...
test('calling config() reconfigures observable API too', t => {
  ...
  client.config({dataset: 'def456'})
  t.equal(
    client.observable.config().dataset,
    'def456',
...
test('can clone client', t => {
  ...
  client2.config({dataset: 'def456'})
  t.equal(client.config().dataset, 'abc123')
  t.equal(client2.config().dataset, 'def456')
  t.end()
})

test('throws if no dataset is set', t => {
  t.throws(lyraClient, /dataset/)
  t.end()
})

test('throws on invalid dataset names', t => {
  t.throws(() => lyraClient({dataset: '*foo*'}), /Datasets can only contain/i)
  t.end()
})
...
test('Patch is available on client and can be used without instantiated client', t => {
  const patch = new lyraClient.Patch('foo.bar')
  t.deepEqual(
    patch
      .inc({foo: 1})
      .dec({bar: 2})
...
test('Transaction is available on client and can be used without instantiated client', t => {
  const trans = new lyraClient.Transaction()
  t.deepEqual(
...
test('transaction can be created without client and passed to mutate()', t => {
  const trx = new lyraClient.Transaction()
  trx.delete('foo')
...
test('uploads images with progress events', t => {
  ...
  getClient()
    ...
    .subscribe(event => t.equal(event.type, 'progress'), ifError(t), () =>
      t.end()
    )
})
...
test('delete assets given whole asset document', t => {
  ...
  const doc = {_id: 'image-abc123_foobar-123x123-png', _type: 'lyra.imageAsset'}
  getClient()
...
test('can get an image URL from a reference ID string', t => {
  ...
  t.equal(url, 'https://api.lyra.url/images/foo/someImageId-200x300.png')
  t.end()
})
...
test('can get an image URL with added query string', t => {
  ...
  const base = 'https://api.lyra.url/images/foo/someImageId-200x300.png'
...
// Don't rely on this unless you're working at Lyra Inc ;)
...
test('ClientError includes message in stack', t => {
  ...
  const error = new LyraClient.ClientError({statusCode: 400, headers: {}, body})
...
test('ServerError includes message in stack', t => {
  ...
  const error = new LyraClient.ClientError({statusCode: 504, headers: {}, body})
...
test('exposes ClientError', t => {
  t.equal(typeof lyraClient.ClientError, 'function')
  const error = new LyraClient.ClientError({
    statusCode: 400,
    headers: {},
    body: {}
  })
  t.ok(error instanceof Error)
  t.ok(error instanceof lyraClient.ClientError)
  t.end()
})

test('exposes ServerError', t => {
  t.equal(typeof lyraClient.ServerError, 'function')
  const error = new LyraClient.ServerError({
    statusCode: 500,
    headers: {},
    body: {}
  })
  t.ok(error instanceof Error)
  t.ok(error instanceof lyraClient.ServerError)
  t.end()
})

// Don't rely on this unless you're working at Lyra Inc ;)
test('exposes default requester', t => {
  t.equal(typeof lyraClient.requester, 'function')
  t.end()
})
