...
export default function resolvePathFromState(
  node: Node,
  state: Object
): string {
  ...
  if (match.nodes.length === 0) {
    throw new Error(
      `Unable to resolve path from given state: ${JSON.stringify(state)}`
    )
  }
  ...
  const relative = flatten(
    match.nodes.map(matchNode => {
      ...
      return matchNode.route.segments.map(segment => {
        ...
        const transform =
          matchNode.transform && matchNode.transform[segment.name]
        return transform
