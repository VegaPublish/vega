...
import {getDevServer} from '@lyra/server'
import getConfig from '@lyra/util/lib/getConfig'
...
export default async (args, context) => {
  ...
  const lyraConfig = getConfig(workDir)
  const config = lyraConfig.get('server')
  ...
  const serverOptions = {
    ...
    httpPort,
    context,
    project: lyraConfig.get('project')
  }
  ...
  compiler.plugin('done', stats => {
    ...
    if (!hasErrors && !hasWarnings) {
      output.print(
        chalk.green(
          `Content Studio successfully compiled! Go to http://${httpHost}:${httpPort}`
...
function gracefulDeath(httpHost, config, err) {
  if (err.code === 'EADDRINUSE') {
    throw new Error(
      'Port number is already in use, configure `server.port` in `lyra.json`'
    )
  }
