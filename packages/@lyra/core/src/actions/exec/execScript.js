...
module.exports = async args => {
  ...
  const script =
    args.argsWithoutOptions[0] || args.extOptions['with-user-token']
  const withToken = Boolean(args.extOptions['with-user-token'])
  const scriptPath = path.resolve(script)

  if (!script) {
    throw new Error('SCRIPT must be provided. `lyra exec <script>`')
  }
  ...
  const loader = require.resolve('@lyra/plugin-loader/register')
