...
import {getWebpackCompiler, getDocumentElement, ReactDOM} from '@lyra/server'
...
export default async (args, context) => {
  ...
  spin = output.spinner('Building Lyra').start()
  ...
  try {
    ...
    spin.text = `Building Lyra (${stats.time}ms)`
    spin.succeed()
    ...
    if (!compilationConfig.skipMinify) {
      ...
      spin.text = `Minifying Javascript bundles (${Date.now() -
        compressStart}ms)`
      spin.succeed()
    }
