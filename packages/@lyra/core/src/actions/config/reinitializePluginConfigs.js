...
import resolveTree from '@lyra/resolver'
...
import {
  ...
  setChecksums,
  localConfigExists
} from '../../util/pluginChecksumManifest'
...
async function reinitializePluginConfigs(options, flags = {}) {
  ...
  const pluginsWithDistConfig = (await Promise.all(
    allPlugins.map(pluginHasDistConfig)
  )).filter(Boolean)
  const distChecksums = await Promise.all(
    pluginsWithDistConfig.map(getPluginConfigChecksum)
  )
  const withLocalConfigs = await Promise.all(distChecksums.map(hasLocalConfig))
  const missingConfigs = await Promise.all(
    withLocalConfigs.map(createMissingConfig)
  )
  ...
  function createMissingConfig(plugin) {
    ...
    const dstPath = path.join(
      workDir,
      'config',
      `${normalizePluginName(plugin.name)}.json`
    )
    ...
    if (!flags.quiet) {
      output.print(
        `Plugin "${
          plugin.name
        }" is missing local configuration file, creating ${prtPath}`
      )
    }
  ...
  function warnOnDifferingChecksum(plugin) {
    ...
    // See https://github.com/lyra-io/lyra/pull/298
    // if (flags.quiet) {
    //   return plugin
    // }
    //
...
export async function tryInitializePluginConfigs(options, flags = {}) {
  try {
    ...
    const depName =
      err.plugin[0] === '@' ? err.plugin : `lyra-plugin-${err.plugin}`
    if (dependencies.includes(depName)) {
      err.message = `${err.message}\n\nTry running "lyra install"?`
    } else {
      err.message = `${err.message}\n\nTry running "lyra install ${depName}"?`
    }
...
function getPluginConfigChecksum(plugin) {
  return generateConfigChecksum(getPluginConfigPath(plugin)).then(
    configChecksum => Object.assign({}, plugin, {configChecksum})
  )
}
