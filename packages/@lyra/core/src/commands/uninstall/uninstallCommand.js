...
import readLocalManifest from '@lyra/util/lib/readLocalManifest'
...
export default {
  name: 'uninstall',
  signature: '[plugin]',
  description: 'Removes a Lyra plugin from the current Lyra configuration',
  action: (args, context) => {
...
async function uninstallPlugin(plugin, context) {
  ...
  const isFullName = plugin.indexOf('lyra-plugin-') === 0
  const shortName = isFullName ? plugin.substr(14) : plugin
  const fullName = isFullName ? plugin : `lyra-plugin-${plugin}`

  await removeConfiguration(workDir, fullName, shortName, prompt)
  await removeFromLyraManifest(workDir, shortName)
...
async function removeConfiguration(workDir, fullName, shortName, prompt) {
  ...
  try {
    ...
    const {deleteConfig} = await promptOnAlteredConfiguration(
      shortName,
      sameChecksum,
      prompt
    )
...
async function removeFromLyraManifest(workDir, pluginName) {
  const manifest = await readLocalManifest(workDir, 'lyra.json')
  manifest.plugins = without(manifest.plugins || [], pluginName)
  return fse.writeJson(path.join(workDir, 'lyra.json'), manifest, {spaces: 2})
}
