...
import lyraImport from '@lyra/import'
...
Examples
  ...
  lyra dataset import moviedb.ndjson moviedb
  ...
  lyra dataset import moviedb.tar.gz moviedb --replace

  # Import from a folder containing an ndjson file, such as an extracted tarball
  # retrieved through "lyra dataset export".
  lyra dataset import ~/some/folder moviedb

  # Import from a remote URL. Will download and extract the tarball to a temporary
  # location before importing it.
  lyra dataset import https://some.url/moviedb.tar.gz moviedb --replace
`
...
export default {
  ...
  action: async (args, context) => {
    ...
    if (!file) {
      throw new Error(
        `Source file name and target dataset must be specified ("lyra dataset import ${chalk.bold(
          '[file]'
        )} [dataset]")`
      )
    }
    ...
    if (isUrl) {
      ...
      inputStream = sourceIsFolder
        ? sourceFile
        : await fse.createReadStream(sourceFile)
    }
    ...
    // Start the import!
    try {
      const imported = await lyraImport(inputStream, {
        client: importClient,
        operation,
        onProgress
      })
