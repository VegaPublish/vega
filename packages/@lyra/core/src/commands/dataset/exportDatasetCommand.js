...
import {pathTools} from '@lyra/util'
import exportDataset from '@lyra/export'
...
Examples
  lyra dataset export moviedb localPath.tar.gz
  lyra dataset export moviedb assetless.tar.gz --no-assets
  lyra dataset export staging staging.tar.gz --raw
  lyra dataset export staging staging.tar.gz --types products,shops
`
...
export default {
  ...
  action: async (args, context) => {
    ...
    if (outputPath !== '-') {
      output.print(
        `Exporting dataset "${chalk.cyan(dataset)}" to "${chalk.cyan(
          outputPath
        )}"`
      )
    }
    ...
    const onProgress = progress => {
      if (progress.step !== currentStep) {
        ...
        spinner.text = `${progress.step} (${progress.current}/${
          progress.total
        })`
      }
...
async function getOutputPath(destination, dataset, prompt, flags) {
  ...
  const finalPath = looksLikeFile
    ? dstPath
    : path.join(dstPath, `${dataset}.tar.gz`)
