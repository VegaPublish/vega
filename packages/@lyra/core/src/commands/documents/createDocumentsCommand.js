...
Examples
  ...
  lyra documents create myDocument.json

  # Open configured $EDITOR and create the specified document(s)
  lyra documents create
  ...
  lyra documents create --id myDocId --replace
  ...
  lyra documents create --id myDocId --watch --replace --json5
`
...
export default {
  ...
  action: async (args, context) => {
    ...
    const tmpFile = path.join(os.tmpdir(), 'lyra-cli', `${docId}.${ext}`)
...
function validateDocument(doc, index, arr) {
  ...
  if (typeof doc._type !== 'string') {
    throw new Error(
      getErrorMessage(
        'must have a `_type` property of type string',
        index,
        isSingle
      )
    )
...
function getErrorMessage(message, index, isSingle) {
  return isSingle
    ? `Document ${message}`
    : `Document at index ${index} ${message}`
}
