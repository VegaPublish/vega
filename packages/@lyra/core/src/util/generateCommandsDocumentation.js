...
export function generateCommandsDocumentation(
  ...
  const rows = [
    'usage: lyra [-v|--version] [-d|--debug] [-h|--help] <command> [<args>]',
    '',
    'Commands:'
  ]
    .concat(
      commandGroup.map(
        cmd => `   ${padEnd(cmd.name, cmdLength)} ${cmd.description}`
      )
    )
    .concat([
      '',
      "See 'lyra help <command>' for specific information on a subcommand."
    ])
...
export function generateCommandDocumentation(command, group, subCommand) {
  if (!command) {
    throw new Error(
      subCommand
        ? `"${subCommand}" is not a subcommand of "${group}". See 'lyra help ${group}'`
  ...
  return [
    `usage: lyra ${cmdParts} ${command.signature}`,
    '',
    `   ${command.description}`,
    '',
