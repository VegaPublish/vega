...
import resolveLyraRoot from './resolveProjectRoot'
import removeDuplicatePlugins from './removeDuplicatePlugins'

export default resolveTree
export const resolveProjectRoot = resolveLyraRoot
...
async function resolveTreeAsync(options) {
  ...
  const withRoot = plugins.concat([
    getProjectRootPlugin(options.basePath, projectManifest)
  ])
...
function resolveTree(opts = {}) {
  ...
  if (options.resolveProjectRoot) {
    ...
    options.basePath = resolveLyraRoot(resolveOpts)
  }
    ...
    .map(plugin => {
      ...
      if (!Array.isArray(plugin.manifest.parts)) {
        ...
        throw new Error(
          `Plugin "${
            plugin.name
          }" has a "parts" property which is not an array\n${help}`
        )
      }
...
function assignNonOverridablePart(
  plugin,
  part,
  implementations,
  definitions,
  ...
  implementations[part.name] = [
    getImplementationDeclaration(plugin, part, options)
  ]
}
...
function assignPartImplementation(
  plugin,
  part,
  implementations,
  definitions,
...
function getImplementationDeclaration(plugin, part, options) {
  ...
  const isLib =
    options.useCompiledPaths ||
    plugin.path.split(path.sep).indexOf('node_modules') !== -1
