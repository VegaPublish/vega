...
function resolvePluginPath(plugin, sync) {
  const pluginDir =
    plugin.name[0] === '@' ? plugin.name : `lyra-plugin-${plugin.name}`
  ...
  if (plugin.parentPluginPath) {
    locations.splice(
      2,
      0,
      path.join(plugin.parentPluginPath, 'node_modules', pluginDir)
    )
  }
...
function readPluginName(parent, dir) {
  ...
  const manifestPath = path.join(parent, dir, 'lyra.json')
  const baseError = `Plugin/project at "${parent}" has the path "${dir}" as a plugin, but Lyra was not able to load`
  const info = {}
  try {
    info.name = readJson(pkgPath).name.replace(/^lyra-plugin-/, '')
  } catch (err) {
  ...
  try {
    ...
    throw new Error(
      `${baseError} "${manifestPath}" in order to get the plugin parts`
    )
  }
