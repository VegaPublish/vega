...
export default function validateManifest(manifest, plugin) {
  ...
  if (hasPlugins && !Array.isArray(manifest.plugins)) {
    throw new Error(
      `Plugin "${plugin}" has a non-array "plugins" property. Must be an array.`
    )
  }

  if (hasPlugins && manifest.plugins.some(hasDuplicate)) {
    throw new Error(
      `Plugin "${plugin}" has duplicate values in its "plugins" property.`
    )
  }

  if (isDefined(manifest.parts) && !Array.isArray(manifest.parts)) {
    throw new Error(
      `Plugin "${plugin}" has a non-array "parts" property. Must be an array.`
    )
  }
