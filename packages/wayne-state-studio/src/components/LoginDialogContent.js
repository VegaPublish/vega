...
function providersByDataset(providers, dataset) {
  if (dataset === 'lpf') {
    return providers
  }
  return providers.filter(provider => !provider.name.startsWith('lpf-'))
}
    ...
    default:
      return function CustomLogo() {
        return provider.logo ? (
          <img src={provider.logo} className={styles.fallbackProviderImage} />
        ) : (
          <QuestionmarkLogo />
        )
      }
...
export default class LoginDialogContent extends React.Component {
  ...
  handleLoginButtonClicked = (provider, event) => {
    ...
    if (onLoginButtonClick) {
      ...
      const warning =
        'LoginDialogContent is missing the onLoginButtonClick property'
      console.warn(warning) // eslint-disable-line no-console
    }
  }

  render() {
    const {description, providers, LyraLogo} = this.props
    const filteredProviders = providersByDataset(
      providers,
      lyraClient.clientConfig.dataset
    )
    ...
    return (
      <div className={styles.root}>
        <div className={styles.inner}>
          ...
          <h2 className={styles.title}>Choose Your Vega Login</h2>
          {description && (
          ...
          <ul className={styles.providers}>
            {filteredProviders.map(provider => {
              ...
              const onLoginClick = this.handleLoginButtonClicked.bind(
                this,
                provider
              )
              return (
